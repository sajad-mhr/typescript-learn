...
let students: {
  ...
  firstName: string;
  lastName: string;
  scores: { id: number; scoreName: string; score: number }[];
  avarage: number;
}[] = [];
let state: boolean = confirm(
  "به سیستم مدیریت دانش آموزان خوش آمدید. برای ادامه دکمه OK و برای خروج دکمه Cancel را فشار دهید."
);
for (let i: number = 0; state; i++) {
  let f_name: string = prompt(`نام دانش آموز را وارد کنید : `);
  let l_name: string = prompt(`نام خانوادگی دانش آموز را وارد کنید : `);
  let inputScoreCount: number = +prompt(
    "نمره چند درس را میخواهید وارد کنید؟ : "
  );
  let scoresArr: { id: number; scoreName: string; score: number }[] = [];
  for (let j: number = 0; j < inputScoreCount; j++) {
    let inputScoreName: string = prompt(`نام درس را وارد کنید :`);
    let inputScore: number = +prompt(`نمره درس را وارد کنید :`);
    scoresArr[j] = {
      id: j,
...
function CalcAvarage(
  ...
  let avarage: number = +(sum / scores.length).toFixed(2);
  return avarage;
}
